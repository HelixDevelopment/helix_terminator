// Package migrations embeds port-forward-service's SQL schema
// migrations and applies them to PostgreSQL at process startup using
// golang-migrate (github.com/golang-migrate/migrate/v4) with the pgx/v5
// database driver and the iofs (embed.FS) source driver.
//
...
// never an error. This replicates the merged S14 golang-migrate pattern
// (auth-service pilot; analytics-service + audit-service + BATCH D
// wiring) so migrations previously assumed pre-applied are actually run
// by the service binary.
//
// This service has TWO forward migrations (001_init, 002_add_tunnel_
// fields) - each carries its own dedicated *.down.sql reversing ONLY
// that migration's forward changes (002's down drops the 6
// real-SSH-tunnel columns + the 3 CHECK constraints + the index it
// added, restoring the pre-002 status DEFAULT 'active' with no CHECK
// constraint; 001's down drops the port_forwards table entirely), so
...
//     own tables, which collide the instant a second service's migrator
//     runs against the shared database ("relation already exists").
//
// (2) is closed by schema-per-service: every service migrates into its
// own dedicated PostgreSQL schema (see the Schema constant below)
// instead of the shared "public" schema, selected via the standard
// PostgreSQL "search_path" connection parameter.
//
// This package is purely additive schema setup; it does not touch this
// service's real SSH-tunnel client logic (internal/sshclient,
// internal/forwarder) or handler/repository behaviour.
package migrations
...
// across every service sharing the "helixterminator" database.
const Schema = "port_forward_service"
...
// makes this unique per-service on its own, but the service-qualified
// name is kept as defense-in-depth against a future schema-isolation
// regression.
const migrationsTable = "port_forward_service_schema_migrations"

// Logger is the minimal logging interface Run needs. *log.Logger and the
// service-local Logger interfaces both satisfy it.
