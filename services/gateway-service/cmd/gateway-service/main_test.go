...
// main()'s real behaviour (route wiring, upstream health-check prober
// start/stop, graceful shutdown) is covered by internal/server's own
// tests plus integration/e2e tests that boot the actual binary - this
// stub deliberately does NOT assert a tautology. Mirrors the identical,
// already-reviewed §11.4.3 pattern used by org-service, workspace-service,
// pki-service, config-service, and user-service's own cmd/*/main_test.go.
