...
// that need a real database. This duplicates the essential setup from
// the auth-service testutil because the stress/chaos tags are orthogonal
// to the integration tag.
package testutil
...
import (
	...
	"github.com/helixdevelopment/ssh-proxy-service/migrations"
)
...
// via rootless podman, applies ssh-proxy-service migrations, and returns
...
func StartTestPostgres(t *testing.T) (dbURL string, available bool) {
	...
	name := fmt.Sprintf("ssh-proxy-stress-%d", time.Now().UnixNano())
