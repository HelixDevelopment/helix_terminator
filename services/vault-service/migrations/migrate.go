// Package migrations embeds vault-service's SQL schema migrations and
...
// service binaries.
//
// NOTE (vault-service handles secrets): this package's Run touches
// ONLY schema setup (extension/tables/indexes/triggers) - it never
// reads, writes, or transforms any secret material. The
// secrets.encrypted_value / secret_versions.encrypted_value columns
// are created as opaque TEXT columns; encryption-at-rest of their
// contents is applied entirely by this service's own repository/crypto
...
//     own tables (e.g. this service's "secrets"), which collide the
//     instant a second service's migrator runs against the shared
...
// (<service>_service, e.g. auth_service, vault_service) -
...
// regression.
const migrationsTable = "vault_service_schema_migrations"
