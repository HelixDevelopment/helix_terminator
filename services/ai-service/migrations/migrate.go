// Package migrations embeds ai-service's SQL schema migrations and
...
// (table/index/trigger DDL for ai_requests). ai-service's LLM-provider
// dispatch (internal/llmclient) and handler/runtime logic are untouched
// by this change - the migrator only owns getting the schema in place
// before the service's steady-state repository queries run against it.
//
...
// (<service>_service, e.g. auth_service, ai_service) -
...
// regression.
const migrationsTable = "ai_service_schema_migrations"
