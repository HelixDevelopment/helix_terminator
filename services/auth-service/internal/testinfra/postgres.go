...
// auth-service's integration and security test suites (build-tagged
// "integration"). It boots a real, rootless-podman PostgreSQL 17.2
// container, applies auth-service's real golang-migrate schema against
// it via migrations.Run, and hands back a ready-to-use DATABASE_URL.
//
// Per Constitution §11.4.27 (mocks/stubs/fakes are permitted only in
// unit tests) every test importing this package exercises a real
// PostgreSQL instance and the real migration runner - never an
...
func StartPostgres(t *testing.T) string {
	...
	name := fmt.Sprintf("auth-svc-it-%d", time.Now().UnixNano())
	...
	t.Fatalf("failed to apply real auth-service migrations against the test database after 5 attempts: %v", runErr)
	return ""
}
