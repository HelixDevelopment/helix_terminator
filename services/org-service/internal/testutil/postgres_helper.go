...
// that need a real database. Uses rootless podman to boot a disposable
// PostgreSQL 17.2 container and applies org-service migrations.
//
// Constitution §11.4.85 (stress + chaos test mandate) requires tests
// to exercise the real system against real infrastructure.
package testutil
...
// via rootless podman, applies org-service migrations, and returns a
...
func StartTestPostgres(t *testing.T) (dbURL string, available bool) {
	...
	name := fmt.Sprintf("org-stress-%d", time.Now().UnixNano())
