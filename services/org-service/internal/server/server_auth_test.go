...
// T19 (§11.4.214): org-service's authMiddleware previously FAILED OPEN —
...
// handler, so every /api/v1/orgs route was reachable completely unauthenticated.
// These tests reproduce that hole (RED against the pre-fix code) and then guard
// the fail-closed behaviour (GREEN post-fix), matching the canonical Ed25519
// JWT_PUBLIC_KEY chain the keychain/vault/notification services already enforce.
// In-test-generated Ed25519 keypairs only — never a committed key (§11.4.10).
...
func TestAuthMiddleware_RejectsMissingToken(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodGet, "/api/v1/orgs", nil)
...
func TestAuthMiddleware_AppliesToEveryOrgRoute(t *testing.T) {
	t.Setenv("JWT_PUBLIC_KEY", validTestPublicKeyB64(t))
	srv, err := server.New(nil)
	require.NoError(t, err)
	...
	routes := []struct {
		...
		{http.MethodGet, "/api/v1/orgs"},
		{http.MethodGet, "/api/v1/orgs/" + id},
		{http.MethodGet, "/api/v1/orgs/by-slug/some-slug"},
		{http.MethodPut, "/api/v1/orgs/" + id},
		{http.MethodDelete, "/api/v1/orgs/" + id},
		{http.MethodPost, "/api/v1/orgs/" + id + "/teams"},
		{http.MethodGet, "/api/v1/orgs/" + id + "/teams"},
		{http.MethodGet, "/api/v1/teams/" + id},
		{http.MethodPut, "/api/v1/teams/" + id},
		{http.MethodDelete, "/api/v1/teams/" + id},
		{http.MethodPost, "/api/v1/orgs/" + id + "/members"},
		{http.MethodGet, "/api/v1/orgs/" + id + "/members"},
		{http.MethodPut, "/api/v1/orgs/" + id + "/members/" + userID},
		{http.MethodDelete, "/api/v1/orgs/" + id + "/members/" + userID},
	}
	...
	for _, rt := range routes {
		t.Run(rt.method+" "+rt.path, func(t *testing.T) {
			...
			srv.Router().ServeHTTP(w, req)
			assert.Equal(t, http.StatusUnauthorized, w.Code,
				"%s %s must require a valid JWT like every other /api/v1/orgs route", rt.method, rt.path)
		})
	}
}
