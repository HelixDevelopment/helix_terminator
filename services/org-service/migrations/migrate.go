...
//     the shared database ("relation already exists"). org-service
//     itself declares no "users" table today, but is migrated into
//     its own schema for the same reason (organizations/teams/
//     memberships) and for consistency with the rest of the fleet.
//
...
// schema has been selected to create in"). This also ensures
// CREATE EXTENSION IF NOT EXISTS "uuid-ossp" (001_init.up.sql) creates
// its objects inside Schema, not "public", since Schema is the first
// (only) entry in search_path by the time that statement runs.
//
