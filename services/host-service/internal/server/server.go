...
import (
	...
	"github.com/helixdevelopment/host-service/migrations"
)
...
// services untouched, so host-service independently validates the SAME token
...
// Server wraps the Gin engine with host service functionality.
type Server struct {
	router       *gin.Engine
	logger       Logger
...
func New(logger Logger) (*Server, error) {
	...
	if dbURL != "" {
		...
		if version, merr := migrations.Run(dbURL, logger); merr != nil {
			...
			// steady-state pool's unqualified "hosts" queries
	...
	api.Use(s.authMiddleware())
	{
		...
		api.GET("/hosts/:id", h.GetHost)
		api.PUT("/hosts/:id", h.UpdateHost)
		api.DELETE("/hosts/:id", h.DeleteHost)
		api.POST("/hosts/:id/test-connection", h.TestConnection)
		api.GET("/hosts/:id/logs", h.GetConnectionLogs)
	}
...
func (s *Server) loggingMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		s.logger.Printf("[HOST] %v | %3d | %13v | %15s | %-7s %s",
...
// authMiddleware validates the caller's bearer JWT on every /api/v1/hosts
