...
//
// This is adapted from auth-service's testutil/postgres_helper.go,
// wired to host-service's migrations package (host_service schema).
package testutil
...
// via rootless podman, applies host-service migrations, and returns a
...
func StartTestPostgres(t *testing.T) (dbURL string, available bool) {
	...
	name := fmt.Sprintf("host-stress-%d", time.Now().UnixNano())
