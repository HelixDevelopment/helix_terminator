...
//
// This duplicates the essential setup pattern from auth-service's
// testutil because the stress/chaos tags are orthogonal to the
// integration tag.
package testutil
...
// via rootless podman, applies collaboration-service migrations, and
...
func StartTestPostgres(t *testing.T) (dbURL string, available bool) {
	...
	name := fmt.Sprintf("collab-stress-%d", time.Now().UnixNano())
