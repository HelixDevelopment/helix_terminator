...
//
// This duplicates the essential setup from the auth-service pattern
...
// via rootless podman, applies billing-service migrations, and returns a
...
func StartTestPostgres(t *testing.T) (dbURL string, available bool) {
	...
	name := fmt.Sprintf("billing-stress-%d", time.Now().UnixNano())
