// Package migrations embeds pki-service's SQL schema migrations and
...
// never an error. This mirrors the auth-service pilot (GAP-02): migrations
// were previously assumed pre-applied and never actually run by the
// service binaries.
//
// NOTE (pki-service handles PKI private keys): this package's Run
// touches ONLY schema setup (tables / indexes / triggers) - it never
// reads, writes, or transforms any certificate/key material. The
// certificate_authorities.ca_key_pem and certificates.key_pem columns
// are created as opaque TEXT columns; any encryption-at-rest of their
// contents is applied by this service's own repository/crypto layer
// (PKI_ENCRYPTION_KEY), entirely outside this package.
//
// All helix_terminator services share a single PostgreSQL database (see
// infrastructure/docker/compose/docker-compose.yml, POSTGRES_DB defaulting
// to "helixterminator"). Two independent collision surfaces follow from
// that:
//
...
//     own tables (e.g. this service's "certificates"), which collide
//     the instant a second service's migrator runs against the shared
...
// (<service>_service, e.g. auth_service, pki_service) -
// EnsureSchema/ConnectionURL/Schema unchanged in shape, only the Schema
// and migrationsTable constants differ per service.
package migrations
...
// regression.
const migrationsTable = "pki_service_schema_migrations"
