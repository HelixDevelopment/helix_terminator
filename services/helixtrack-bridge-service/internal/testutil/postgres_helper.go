...
// that need a real database but cannot import testinfra (which is gated
// behind //go:build integration).
//
// This duplicates the essential setup from internal/testinfra/postgres.go
// because the stress/chaos tags are orthogonal to the integration tag
// and the task constraint forbids modifying existing files.
package testutil
...
import (
	...
	"github.com/helixdevelopment/helixtrack-bridge-service/migrations"
)
...
// via rootless podman, applies helixtrack-bridge-service migrations, and
// returns a schema-scoped DATABASE_URL ready for pgxpool.New. The
...
func StartTestPostgres(t *testing.T) (dbURL string, available bool) {
	...
	name := fmt.Sprintf("htbridge-stress-%d", time.Now().UnixNano())
