// Package migrations embeds container-bridge-service's SQL schema
// migrations and applies them to PostgreSQL at process startup using
// golang-migrate (github.com/golang-migrate/migrate/v4) with the
// pgx/v5 database driver and the iofs (embed.FS) source driver.
//
// Scope note: this package is purely additive schema-per-service DDL
// (table/index DDL for container_bridges). container-bridge-service's
// container-runtime detection + handler/runtime logic
// (internal/containerrt, internal/handler) is untouched by this change
// - the migrator only owns getting the schema in place before the
// service's steady-state repository queries run against it.
//
...
// never an error. This closes the same class of gap already remediated
// for auth-service/analytics-service/audit-service: migrations were
...
//     own tables, which collide the instant a second service's
//     migrator runs against the shared database ("relation already
//     exists").
//
...
// (<service>_service, e.g. auth_service, container_bridge_service) -
...
const Schema = "container_bridge_service"
...
// regression.
const migrationsTable = "container_bridge_service_schema_migrations"
