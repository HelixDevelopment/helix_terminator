...
func New(logger Logger) (*Server, error) {
	...
	if dbURL != "" {
		...
		// creation and fall through to degraded mode below, matching this
		...
		if version, merr := migrations.Run(dbURL, logger); merr != nil {
			...
			// steady-state pool's unqualified "terminal_sessions"
			// queries resolve against the schema migrations.Run just
			// migrated, not the shared database's default "public"
			// schema (schema-per-service, GAP-01).
